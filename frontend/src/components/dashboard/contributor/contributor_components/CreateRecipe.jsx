...
import { createRecipeAction } from "../../../../redux/slice/recipesSlices";
...
const CreateRecipe = () => {
  ...
  const { loading, error, isAdded } = useSelector((state) => state?.recipes);
  ...
  useEffect(() => {
    if (isAdded) {
      ...
      if (quillInstructionInstance.current) {
        quillInstructionInstance.current.setContents([]);
      }
      if (quillIngredientInstance.current) {
        quillIngredientInstance.current.setContents([]);
      }
    }
  }, [isAdded]);
    ...
    <div className="flex justify-center">
      ...
      <div className="w-[37%] shadow-2xl my-5">
        <form className="p-6 shadow  bg-white rounded-2xl">
          <h2 className="mb-4 text-2xl text-center">Create Recipe</h2>

          {error && <ErrorMsg message={error?.message} />}
          {isAdded && <SuccessMsg message="Recipe created successfully." />}
